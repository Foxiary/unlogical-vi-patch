...
- `romfs/Data/**` — the patched Unity binaries (`.assets`, bundles, `global-metadata.dat`). These *are* the deliverable. **35 files ship for the base title; 34 are tracked in git** — `font_jp` is gitignored and published as a release asset instead. Three of the 35 (`sprite/sprite02`, `texture/texture02`, `sharedassets24.assets`) exist only for the DLC hub that the base game draws — its art and its character-list row prefab.
