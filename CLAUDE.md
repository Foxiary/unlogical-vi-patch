...
- `romfs/Data/**` — the patched Unity binaries (`.assets`, bundles, `global-metadata.dat`). These *are* the deliverable. **34 files ship for the base title; 33 are tracked in git** — `font_jp` is gitignored and published as a release asset instead. Two of the 34 (`sprite/sprite02`, `texture/texture02`) exist only for the DLC hub's art that the base game draws.
