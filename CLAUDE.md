...
- `tools/fix_novel_prose_break.py` — the novel-mode counterpart of `fix_midphrase_break.py`, which deliberately skips the novel region. `Message(Novel)` (1400 px) indents every *data* line one em and leaves TMP-wrapped lines flush, so a hand-placed break in the middle of a phrase is harmless only while the data line before it fits; once a merge lengthens that line, its tail drops onto an un-indented line and the next hard break indents again (`"Kiến` / `thức cơ bản"` in IMG_7238). The script joins the mid-phrase breaks of the offending message and keeps sentence-boundary ones; `--check` belongs in the post-merge gate. From the same pass, `fix_midphrase_break.py` no longer skips every ruby message outright: `ruby_safe()` allows the join when the ruby still lands on the first rendered line at the real auto-size.
