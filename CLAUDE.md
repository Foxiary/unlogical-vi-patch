...
| `level10` | 0.2 | 4 TMP components (ADV message ×2, novel-mode box, dictionary popup) · the two ADV message `RectTransform`s narrowed 1400 → 1280 so no line slides under the box's corner art · the centre-caption `EXTRAText` `RectTransform` narrowed 1920 → 1764 so TMP wraps inside the watermark margin instead of the data carrying word-level breaks |
...
- `tools/fix_adv_box_width.py` — the one component patch of that family: `level10` pid 564/581 `m_SizeDelta.x` 1400 → 1280, bytes in place, size unchanged. `tools/fix_caption_box_width.py` does the same for the centre caption: `EXTRAText` pid 722, 1920 → 1764, the watermark's safe margin. That retired the word-level second tier of `fix_center_caption_wrap.py`; caption hard breaks are punctuation-only now, and the same string no longer shows a stub line in the SAVE/LOAD summary box.
